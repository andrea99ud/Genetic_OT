...
class Genetic_Algorithm:
    def __init__(self, N, l, w, beta, lambda_star):
        ...
        #print("current cost: ", self.current_cost.shape)
        #print("current cost: ", self.current_cost)
        #print sum of all columns
        #print("columns: ", self.columns.shape)
    ...
    def run(self, max_iter, max_samples=100):
        ...
        for _ in tqdm(range(max_iter)):
            ...
            #print("alpha active: ", alpha_active.shape)
            #print("alpha active: ", alpha_active)
            #active indices are where alpha active is > 0
            self.active_set_indices = np.where(alpha_active > 0)[0]
            #print("active set indices intermediate: ", self.active_set_indices.shape)
            #print("active set indices: ", self.active_set_indices.shape)
            self.y_star = self.solve_DRMP()
            #print("y star: ", y_star.shape)

            while gain < 0:
                #random element from active set
                ...
                #print("cost child: ", cost_child.shape)
                #print("cost child: ", cost_child)
            ...
            if self.columns.shape[0] >= self.l*(self.beta+1):
                ...
                #print("number of columns: ", self.columns.shape[0])
                non_active_indices = np.setdiff1d(np.arange(0, self.columns.shape[0], 1), self.active_set_indices)[:self.l]
                #print("non active indices: ", non_active_indices.shape)
                #print("non active indices: ", non_active_indices)
                #print("active set indices: ", self.active_set_indices.shape)
                #print("active set indices: ", self.active_set_indices)
                self.columns = np.delete(self.columns, non_active_indices, axis=0)
                #print("columns: ", self.columns.shape)
                #print("current cost: ", self.current_cost.shape)
                alpha_active = np.delete(alpha_active, non_active_indices, axis=0)
                self.current_cost = np.delete(self.current_cost, non_active_indices, axis=0)
                #print("current cost: ", self.current_cost.shape)
    ...
    def solve_RMP(self, alpha_active):
        if alpha_active is None:
            alpha = sp.optimize.linprog(c=self.current_cost, A_eq=self.columns.T, b_eq=self.lambda_star, method="highs", bounds=(0, None))
        #minimize current_cost*alpha subject to alpha*columns = lambda_star and alpha >= 0
        else:
            alpha_active = np.append(alpha_active,0)
            #print("alpha active: ", alpha_active.shape)
            #print("current cost: ", self.current_cost.shape)
            alpha = sp.optimize.linprog(c=self.current_cost, A_eq=self.columns.T, b_eq=self.lambda_star, method="highs", bounds=(0, None), x0=alpha_active)
        print("cost", alpha.fun)
        return alpha.x

    def solve_DRMP(self):
        #maximize y*lambda_star subject to columns*y <= current_cost
        y = sp.optimize.linprog(c=-self.lambda_star, A_ub=self.columns, b_ub=self.current_cost, method="highs" )
        return y.x
    def initialize_cost_lambda(self, column):
        cost_lambda = np.array([(N**2/2)*i.dot(self.cost_matrix).dot(i) - (N/2)*np.diag(self.cost_matrix).dot(i) for i in column])
        return cost_lambda
    def get_cost_lambda(self, column):
        cost_lambda = (N**2/2) * column.dot(self.cost_matrix).dot(column) - (N/2)*np.diag(self.cost_matrix).dot(column)
        return cost_lambda
    def compute_2_marginal_matrix(self):
        marginal = np.array([self.N/(self.N-1)*np.tensordot(column, column, axes=0)-1/(self.N-1)*np.diag(column) for column in self.columns])
